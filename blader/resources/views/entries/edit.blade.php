<x-layouts.app :title="__('Dashboard')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            <h1 class="my-2 text-4xl font-extrabold">{{ __('Edit Entry') }}</h1>
        </div>
        ...
        <div class="flex flex-col items-center justify-center gap-4 p-6 md:p-10">
            <div class="flex w-full max-w-lg flex-col gap-2">
                <form action="{{ route('entries.store') }}" method="POST" class="flex flex-col gap-6">
                    @csrf
                    <div class="relative">
                        <label
                    ...
                    <div class="flex items-center justify-end">
                        <flux:button variant="primary" type="submit" class="w-full">{{ __('Create Entry') }}</flux:button>
                    </div>
                </form>
            </div>
        </div>
